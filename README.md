...
4. **Interactive Book Gallery:** A "Related Books" section where cards flip on hover to reveal the price and a "Details" button. Clicking the button reveals more book information and a link to purchase on Amazon — implemented entirely with CSS (no JavaScript).
5. **Footer:** A two-column footer containing designer information (copyright) and a set of links with hover effects.
6. **Fixed Navigation Menu:** A custom-styled checkbox acts as a hamburger menu icon (the "checkbox hack"). When clicked, it reveals a smooth, sliding navigation menu that links to the main sections of the page. The menu remains fixed on scroll.
7. **Favicon & Title:** Custom favicon and page title for the browser tab.

## 🛠️ Built With

- **HTML5** - For semantic structure.
- **CSS3** - For styling and interactivity, including:
  ...
  - Pure-CSS interactivity (`:hover`, `:checked`, `:target`) for the card flips and hamburger menu — no JavaScript required
...
├── Capture.PNG          # Screenshot / preview image
└── README.md
```

## 🚀 Running Locally

No build step or dependencies required — this is a static HTML/CSS site.

1. Clone the repository:
```bash
   git clone https://github.com/Amirbehnam1009/Book-Web-Page.git
   cd Book-Web-Page
```
